...
parser.add_argument("--dry-run", "-d", action="store_true", help="print config")
...
if memory_limit:
    settings["process"]["rlimits"] = [
        {"type": "RLIMIT_AS", "hard": int(memory_limit), "soft": int(memory_limit)}
    ]
...
if args.faketime:
    ...
    cmd_args.append("-f")
    cmd_args.append(
        "2020-01-01 00:00:00" if args.faketime == "default" else args.faketime
    )
...
def make_command(root_dir, action):
    ...
    command = (
        [
            "runsc",
            "-root",
            "/tmp/runsc",  # Place container information somewhere writable.
        ]
        + flags
        + ["-network", "none"]
        + action
        + [root_dir.replace("/", "_")]
    )  # Derive an arbitrary container name.
    return command
